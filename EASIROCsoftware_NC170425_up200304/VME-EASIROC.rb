...
# @で始まる変数はインスタンス変数であり、特定の オブジェクトに所属しています。
...
class VmeEasiroc
    def initialize(host, tcp_port, udp_port)
        $logger.debug "\nVmeEasiroc::initialize: host=#{host}, tcp_port=#{tcp_port}, udp_port=#{udp_port}\n"
        @host = host
        @tcp_port = tcp_port
        ...
        @sendAdc = true
        @sendTdc = true
        @sendScl = false # initialization of scaler 
    end

    attr_accessor :easiroc1, :easiroc2
    attr_accessor :sendAdc, :sendTdc, :sendScl
	...
	def sendSelectbaleLogic
      ...
      $logger.info "[sendSelectbaleLogic] data: length = #{selectableLogic.length}  "
      $logger.info "[sendSelectbaleLogic] data: #{selectableLogic}"
	...
	def sendTriggerValues
      ...
      $logger.info "VmeEasiroc::sendTriggerValues address=#{address}, triggerMode=#{triggerMode}"
   ...
   def sendTestChargePattern
      $logger.info "VmeEasiroc::sendTestChargeSetting is called"
		# get constand address 
      address = testChargeAddress 
      $logger.info "VmeEasiroc::sendTestChargeSetting address=#{address}, setting=#{@testCharge}"
   ...
   def setTestChargePattern(setting)
      puts "[setTestChargePattern] val=#{setting}"
		@testCharge = setting.to_i
		sendTestChargePattern
	end 

    def reloadSetting
        $logger.info "VmeEasiroc::reloadSetting Load settings"
        ...
        @testCharge= @configLoader.toTestChargeSetting
    end
	...
	def version
      ...
      combinedData = @rbcp.read(address, 6) # 6-3
    ...
    def sendSlowControlSub(easiroc1, easiroc2)
        ...
        $logger.debug("VmeEasiroc::sendSlowControl: Easiroc1")
        $logger.debug(easiroc1.map{|i| "    %02X\n" % i}.join)
        address = slowControl1Address
        @rbcp.write(address, easiroc1)

        $logger.debug("VmeEasiroc::sendSlowControl: Easiroc2")
    ...
    def writeStatusRegister(data = 0)
        $logger.debug "VmeEasiroc::writeStatusRegister: statusRegisterAddress [ address is defined as 0x00000077 ]"
        address = statusRegisterAddress 
        if @daqMode
            data |= daqModeBit # ビット和 data = data | daqModeBit
        end
        if @sendAdc
        		$logger.debug "VmeEasiroc::writeStatusRegister: sendAdcBit=#{sendAdcBit}"
            data |= sendAdcBit
        end
        if @sendTdc
        		$logger.debug "VmeEasiroc::writeStatusRegister: sendAdcBit=#{sendTdcBit}"
            data |= sendTdcBit
        end
        if @sendScl
        		$logger.debug "VmeEasiroc::writeStatusRegister: sendAdcBit=#{sendSclBit}"
            data |= sendSclBit
        end
        $logger.debug "VmeEasiroc::writeStatusRegister: ====> data=#{data}"
        @rbcp.write(address, data)
    end
    ...
    def exitDaqMode
        $logger.debug "VmeEasiroc::exitDaqMode ===> exit DAQ MODE"
        @daqMode = false
        writeStatusRegister
    end
