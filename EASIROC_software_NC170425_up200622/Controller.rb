...
class CommandDispatcher
  ...
  COMMANDS = %w(
    ...
    statusInputDAC setInputDAC setThreshold setRegister
  ...
  DIRECT_COMMANDS.each do |command|
   ...
   def setHV(value)
      ...
      if diff.abs > 10.0 then
         puts "diff is #{diff.abs}! diff is too large! use stepSetHV"
         return
      end
      ...
      while !check do
         count+=1
      	 ...
      	 if voltage > vollim || current > curlim then
      	    ...
      	    puts sprintf("=> <checkHV> Status OK. The current V = %.2fV, A = %.2fuA", voltage, current)
      	    check=true
      	 end
      end
   end
  ...
  def drawScaler(filename="temp", dac="reg", *ch)
   ...
   def calibrateIndividual64
      ...
      for iteration in 0..63 do
         puts "calibration by test charge pulse for ch: #{iteration}"  

         fname0 = 'calib_chan' + iteration.to_s + '_qptn0'
         setTestPulseTo(iteration) # call slowcontrol  
         setTestPulsePtn(0)
         read(4000, fname0) # read calls slowcontrol in the end as default

         fname1 = 'calib_chan' + iteration.to_s + '_qptn1'
         setTestPulseTo(iteration)
         setTestPulsePtn(1)
         read(4000, fname1) # read calls slowcontrol in the end as default

         fname2 = 'calib_chan' + iteration.to_s + '_qptn2'
         setTestPulseTo(iteration)
         setTestPulsePtn(2)
         read(4000, fname2) # read calls slowcontrol in the end as default
 
         # convert dat to tree 
         #       cmdtree/tree data/calib_chan0_qptn0.dat
         fname3 = 'sumcalib_chan' + iteration.to_s + '.root'
         fname4 = 'calib_chan' + iteration.to_s + '*.root'
         puts "\nmv data/calib_chan* and cmdtree/tree \n"
         system("mv data/calib_chan* data_calib/")
         system("cmdtree/tree data_calib/#{fname0}")
         system("cmdtree/tree data_calib/#{fname1}")
         system("cmdtree/tree data_calib/#{fname2}")
         puts "\nhadd -f data_calib/#{fname3} data_calib/#{fname4}\n"
         system("hadd -f data_calib/#{fname3} data_calib/#{fname4}")
      end
   end
   ...
   def activateIndividual64
      for iteration in 0..63 do
         puts "activate only #{iteration}"
         setSelectbaleLogic(iteration)
         sleep 1.5
      end
   end
  ...
  Keep a Diary on SFGD-64PCB 
   ...
   - @ 20/06/17 Config: EASIORC(KEK15, ch0--4 are ploblematic)-Splitter-64PCB, firmware 80 & 1000 nsec
   ...
   - @ 20/06/21 Config: EASIORC(KEK15, ch0--4 are ploblematic)-Splitter-64PCB, firmware 80 &  600 nsec
