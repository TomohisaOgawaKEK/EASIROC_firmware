...
class ConfigLoader
  def initialize(registerAttribute, registerValueAlias, defaultRegisterValue,
    ...
    @triggerWidth = @registerValue['TriggerWidth']
  ...
  def toTriggerWidth
    intTrigWidth = 
      ...
      when 'raw'
        ...
        (@triggerWidth.split('ns')[0].to_i-38)/8
      end
    intTrigWidth
  end
  ...
  def validateClass
    ...
    settingNames = ['EASIROC1','EASIROC2',
                    ...
                    'High Gain Channel 1','High Gain Channel 2',
                    'SelectableLogic','TimeWindow','UsrClkOut']
    ...
    settingClasses = [Hash, Hash, 
                      ...
                      Fixnum, Fixnum, Hash] 
    unless settingClasses == settingNames[0..8].map{|i| @registerValue[i].class}
      puts "Error: RegisterValue file structure error"
      raise ""
    end
    
    unless @registerValue['TimeWindow'].class == String
      puts 'Error: RegisterValue file structure error'
      raise ''
    end
    
    unless @registerValue['UsrClkOut'].class == String ||
      @registerValue['UsrClkOut'].class == Fixnum
      puts 'Error: RegisterValue file structure error'
      raise ''
    end
  ...
  def validateTrigWidth
    widthValue = @registerValue['TriggerWidth'].split('ns')[0]
    if widthValue != 'raw'
      unless widthValue.to_i <= 800 && widthValue.to_i >= 40
        puts 'Error: TriggerWidth must be "raw" or "40ns ~ 800ns"'
        raise ''
      end
    end
  end
  ...
  def validateUsrClkOut
    usrClkOutNames = [0,1,'1Hz','10Hz','100Hz',
                      '1kHz','10kHz','100kHz','3MHz'] 
    unless usrClkOutNames.include?(@registerValue['UsrClkOut'])
      puts 'Error: UsrClkOut must be "0, 1, 1Hz, 10Hz, 100Hz, 1kHz, 10kHz, 100kHz, 3MHz"'
      raise ''
    end
  end
