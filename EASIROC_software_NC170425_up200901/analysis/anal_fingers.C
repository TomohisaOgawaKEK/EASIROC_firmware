...
TF1 *DoFitting(TH1F *h1, std::vector <std::pair <double, double> > peaksXY, int NSmooth = 100)
{
    ...
    if (dval<15) dval = peaksXY[1].first - peaksXY[0].first;
    ...
    for (; j<100; j++) {
      ...
      //cerr << "chi2/Ndof = " << chi2/(double)Ndof << endl;
      if ( pe1 < 0.5 && pe4 < 0.5 && pe7 < 1.5 && // depends on #of events for this peak 
           chi2/Ndof < 4.5 ) break;
#endif
   }
   cerr << "  dval = " << dval 
...
void anal_fingers(TString dir = "../data_store/easiroc200625_farm1_led_temp20_10pts_SAMTEC1.0mNo1_Ecable1.0mNo12_UT18/")
{
    ...
    // ----- HV set value
	const int nfs = 11;
    //double voltage[nfs] = {56.60, 56.80, 57.00, 57.20, 57.40 ,57.60};
    //double voltage[nfs] = {56.20, 56.40, 56.60, 56.80, 57.00 ,57.20};
    double voltage[nfs] = {56.2, 56.3, 56.4, 56.5, 56.6, 56.7, 56.8, 56.9, 57.00, 57.1, 57.2};
	...
	for (int i=0; i<nfs; i++) {
        ...
        cerr << i << " returnHV size = " << retHV[i].size() << endl;  
 	}
	...
	TH2F *h2_Vbd = new TH2F("h2_Vbd","h2_Vbd", 64,0,64, 10,0,10);
	TH2F *h2_VbdEr = new TH2F("h2_VbdEr","h2_VbdEr", 64,0,64, 10,0,10);
	...
	//for (int n=0; n<1; n++) {
        ...
        for (int i=sttCh; i<endCh; i++) {
			...
			TF1 *tgaus;
 			...
 			for (int q=0; q<20; q++) {
                for (int p=0; p<2; p++) {
					peaksXY = GetPeakX(h1[n][i], 1000 - 25*q, 5+p);
		  			const int NPeaks = peaksXY.size(); 
        			if ( NPeaks<3 ) continue;

        			h1[n][i]->Draw("same");
        			tgaus = DoFitting(h1[n][i], peaksXY, 1000 - 25*q);
	        		chi2 = tgaus->GetChisquare();
	        		Ndof = tgaus->GetNDF();
	        		pe1  = tgaus->GetParError(1);
	        		pe4  = tgaus->GetParError(4);
	        		...
	        		cerr << "chi2/Ndof = " << chi2/Ndof
	                     ...
	                     << endl;
                     #endif
	            	if( chi2/Ndof<4.5 && pe1<1.0 && pe4<1.0 && pe7<1.5 ) break;
                }
            	if( chi2/Ndof<4.5 && pe1<1.0 && pe4<1.0 && pe7<1.5 ) break;
			}
		    ...
		    int nth = ge[i]->GetN();
#if 0
            ge[i]->SetPoint     (nth, retHV[n][64], d1 );
		    //ge[i]->SetPoint     (nth, retHV[n][64]-retHV[n][i], d1);
            ge[i]->SetPointError(nth,         0.010, de1);
#endif
#if 1 
		    //ge[i]->SetPoint     (nth, voltage[n], (d1 + d2)/2.);
		    //ge[i]->SetPoint     (nth, retHV[n][64], (d1 + d2)/2.);
		    ge[i]->SetPoint     (nth, retHV[n][64]-retHV[n][i], (d1 + d2)/2.);
    ...
    //outf.close();
	cVbd[0]->Print(dir+"breakdown1.pdf");
	cVbd[1]->Print(dir+"breakdown2.pdf");
