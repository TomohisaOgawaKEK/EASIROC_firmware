...
class ModuleCtrlDptr
  ...
  COMMANDS = %w(
    ...
    muxControl slowcontrol standby fit fit_PDE drawScaler dsleep
    read adc tdc scaler cd pwd mode reset help version timeStamp 
    exit quit progress stop makeError exitDAQ) + DIRECT_COMMANDS.map(&:to_s)
  
  def initialize(namedEasiroc, hist, q)
    @namedEasiroc = namedEasiroc
    @hist = hist
    @q = q
  end
  ...
  def shutdownHV
    @namedEasiroc.sendShutdownHV
  end

  def setHV(value)
    @namedEasiroc.sendMadcControl
    @namedEasiroc.sendHVControl(value.to_f)
  end
  ...
  def increaseHV(value)
    ...
    if value < 0.0 then
      ...
      return
    elsif value > 70.0 then #90
      puts "Too large input value! Must be smaller than 70.0" #90
      return
    end
  ...
  def decreaseHV(value)
    ...
    rd_madc = @namedEasiroc.readMadc(3)
  ...
  def statusHV
    ...
    #@namedEasiroc.readMadc(3) #for debug by hosomi 151110
    rd_madc = @namedEasiroc.readMadc(3)
    puts sprintf("#{@namedEasiroc.name}: Bias voltage >> %.2f V", rd_madc)
    
    ## Read the MPPC bias current
    #@namedEasiroc.readMadc(4) #for debug by hosomi 151110
    rd_madc = @namedEasiroc.readMadc(4)
    puts sprintf("#{@namedEasiroc.name}: Bias current >> %.2f uA", rd_madc)
  end
  ...
  def statusTemp
    ...
    rd_madc = @namedEasiroc.readMadc(5)
    puts sprintf("Temparature 1  >> %.2f C", rd_madc)

    ## Read the temparature2
    rd_madc = @namedEasiroc.readMadc(0)
  ...
  def statusInputDAC(channel, filename="temp")
    ...
    if 0<=chInt && chInt<=63
      num = chInt%32
      readNum = chInt/32 + 1
      @namedEasiroc.setCh(num)
      #@namedEasiroc.readMadc(readNum) #for debug by hosomi 151110
      rd_madc = @namedEasiroc.readMadc(readNum)
      ...
      status = {}
      #@namedEasiroc.readMadc(3) #for debug
      status[:HV] = @namedEasiroc.readMadc(3).round(3)
      status[:current] = @namedEasiroc.readMadc(4).round(3)
      status[:InputDAC]=[]
      ch = 0..31
      ch.each{|eachnum|
        @namedEasiroc.setCh(eachnum)
        #@namedEasiroc.readMadc(1) #for debug by hosomi 151110
        #status[:InputDAC] << sprintf("%.3f", @namedEasiroc.readMadc(1))
        status[:InputDAC] << @namedEasiroc.readMadc(1).round(3)
        #rd_madc = @namedEasiroc.readMadc(1)
        #puts sprintf("ch %2d: Input DAC >> %.2f V",eachnum,rd_madc)
      }
      ch = 32..63
      ch.each{|eachnum|
        @namedEasiroc.setCh(eachnum-32)
        #@namedEasiroc.readMadc(2) #for debug by hosomi 151110
        #status[:InputDAC] << sprintf("%.3f", @namedEasiroc.readMadc(2))
        status[:InputDAC] << @namedEasiroc.readMadc(2).round(3)
        #rd_madc = @namedEasiroc.readMadc(2)
        #puts sprintf("ch %2d: Input DAC >> %.2f V",eachnum,rd_madc)
      }
      puts status
  ...
  def checkHV(vollim=80.0, curlim=20.0, repeat=3)
    ...
    while !check do
      ...
      ## Read the MPPC bias voltage
      voltage = @namedEasiroc.readMadc(3)

      ## Read the MPPC bias current
      current = @namedEasiroc.readMadc(4)
  ...
  def setInputDAC(voltage)
    @namedEasiroc.setInputDAC(voltage.to_f)
    sleep 0.5
    slowcontrol
  end

  def setRegister(key, value)
    @namedEasiroc.setRegister(key, value)
    sleep 0.5
    slowcontrol
  end
  
  def setThreshold(pe, chip="0", filename="temp")
    @namedEasiroc.setThreshold(pe, chip, filename)
    sleep 0.5
    slowcontrol
  end

  def muxControl(chnum)
    @namedEasiroc.setCh(chnum.to_i)
  end
  
  def slowcontrol
    @namedEasiroc.reloadSetting
  ...
  def adc(on_off)
    puts "Set adc #{on_off}"
    if(on_off == 'on')
      @namedEasiroc.sendAdc = true
    elsif(on_off == 'off')
      @namedEasiroc.sendAdc = false
    else
  ...
  def tdc(on_off)
    puts "Set tdc #{on_off}"
    if(on_off == 'on')
      @namedEasiroc.sendTdc = true
    elsif(on_off == 'off')
      @namedEasiroc.sendTdc = false
    else
      puts "Unknown argument #{on_off}"
      return
    end
  ...
  def scaler(on_off)
    ...
    if(on_off == 'on')
      @namedEasiroc.sendScaler = true
    elsif(on_off == 'off')
      @namedEasiroc.sendScaler = false
    else
      puts "Unknown argument #{on_off}"
      return
    end
  ...
  def standby(counts)
    counts.to_i.times {
      ...
      #puts "EASIROC #{@namedEasiroc.name} is ready."
    }
	...
	def read(events, filename="temp", mode="default")
   	$logger.debug "Begin of read."
    	...
    	if (!filename.include?("temp") && File.exist?(data_filename))
      	...
      	data_filename="data/temp_#{@namedEasiroc.name}_#{Time.now.to_i}.dat"
    	end
    	...
    	if mode=="default"
      	...
      	File.open(data_filename, 'wb') do |file|
        		#puts "file open"
        		@namedEasiroc.readEvent(events) do |header, data|
      	...
      	puts "here queue #{@namedEasiroc.name}"
      	File.open(data_filename, 'wb') do |file|
      ...
      puts "here enclk #{@namedEasiroc.name}"
      setRegister("clk", "0")
      enclk = Queue.new
      #num_events = Queue.new
      ...
      begin
        ...
        $logger.info "Error in read, mode==enclk, enclk_thread. name: #{@namedEasiroc.name}"
      ...
      begin
        ...
        read_thread = Thread.new {
          ...
          File.open(data_filename, 'wb') do |file|
            enclk.push(1)
            Thread.pass
            @namedEasiroc.readEvent(events, timelimit+10) do |header, data|
              progress_bar ||= ProgressBar.create(
                total: events,
                format: '%p%% [%b>%i] %c %revent/s %e'
              )
        ...
        if isTimeout then
          raise("Timeout Error. name: #{@namedEasiroc.name}")
          read_thread.kill
        end
      rescue => e
        $logger.info "Error in read, mode==enclk, read_thread. name: #{@namedEasiroc.name}"
        $logger.info e.message
      ensure
        #$logger.info "Timeout Error. name: #{@namedEasiroc.name}" if isTimeout
      end
  ...
  def drawScaler(filename="temp", dac="reg", *ch) #hosomi 160126
    if dac == "reg" then
      dac = @namedEasiroc.getRegister("thr")
    else
      dac = dac.to_i
    end
  ...
  def reset(target)
    ...
    if target == 'probe'
      @namedEasiroc.resetProbeRegister
    end
    
    if target == 'readregister'
      @namedEasiroc.resetReadRegister
    end
    
    if target == 'pedestalsuppression'
      @namedEasiroc.resetPedestalSuppression
    end
  end
  ...
  def version
    versionMajor, versionMinor, versionHotfix, versionPatch,
      year, month, day = @namedEasiroc.version
