...
class NamedEasiroc
  ...
  def sendTimeWindow
    ...
    $logger.debug("TimeWindow: #{timeWindow}") #hosomi 151219
  ...
  def config_filename(filename) #hosomi 151117
    specific_filename = "yaml/#{@name}/#{filename}"
    specific_filepath = File.expand_path(specific_filename, File.dirname(__FILE__))
    default_filename = "yaml/#{filename}"
    default_filepath = File.expand_path(default_filename, File.dirname(__FILE__))
    if File.exist?(specific_filepath)
      #puts "Load #{specific_filename}"
      $logger.debug "Load #{specific_filename}"
      return specific_filepath
    else
      #puts "Load #{default_filename}"
      #puts "#{specific_filename} in not found"
      return default_filepath
    end
  end

  def setInputDAC(voltage) #hosomi 151117
  ...
  def setRegister(key, value) #hosomi 151125
  ...
  def reloadSetting
    ...
    #)
    @configLoader = ConfigLoader.new(
      config_filename('RegisterAttribute.yml'),
      config_filename('RegisterValueAlias.yml'),
      config_filename('DefaultRegisterValue.yml'),
      config_filename('RegisterValue.yml'),
      config_filename('InputDAC.yml'),
      config_filename('PedestalSuppression.yml'),
      config_filename('Calibration.yml')
    )
   ...
   def readEvent(numberToRead, timelimit=60*60*48)#60*60
    ...
    def sendHVControl(value)
        ...
        if(value<0.00)
                $logger.info "Input value must be positive!"
                return
        end

          if(value>90.00)
            $logger.info "Too large input value! Must be smaller than 90.00"
            return
          end
          
          hvDAC = (value*hvConst[0] + hvConst[1]).to_i
          $logger.info sprintf("MPPC bias volatage: %.2fV, DAC bit: %d", value, hvDAC)
    ...
    def readMadc(data)
      ...
      sleep 0.05 #for debug by hosomi 151111 #need more than 0.02
      @rbcp.write(address+1,0)

      #Read data
      sleep 0.01 #wait for MADC data #default 1
      #@rbcp.read(address2,1)   ## only for debug...
      #@rbcp.read(address2+1,1) ## only for debug...
      ...
      #puts "readDAC=#{readDAC}" #for degub by hosomi 151110
      $logger.debug("readDAC=#{readDAC}") #for degub by hosomi 151128

      if ( data == 3 )
        #read_madc = madcConst["HV"]*readDAC
        read_madc = madcConst["HV"]*readDAC + madcConst["HVOffset"] #add offset hosomi 20151009
      elsif data == 4
