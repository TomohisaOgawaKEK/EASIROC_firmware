...
class ConfigLoader
  def initialize(registerAttribute, registerValueAlias, defaultRegisterValue,
                 registerValue, inputDAC, pedestalSuppressionValue,
                 calibration)
  ...
  def getEasiroc1SlowControl ###
    return @easiroc1SlowControl
  end

  def getEasiroc2SlowControl ###
    return @easiroc2SlowControl
  end
 
  def setEasiroc1SlowControl__(key, value) ###
    if @easiroc1SlowControl.has_key?(key) then
      puts "setEasiroc1SlowControl__ key is valid. #{key}"
      print "  current value by yaml:\n   "
      p @easiroc1SlowControl.values_at(key)
      puts  " => #{value.inspect}"
      #puts value
      @easiroc1SlowControl.store(key, value)
    end
  end

  def setEasiroc2SlowControl__(key, value) ###
    if @easiroc2SlowControl.has_key?(key) then
      puts "setEasiroc2SlowControl__ key is valid. #{key}"
      print "  current value by yaml:\n   "
      p @easiroc2SlowControl.values_at(key)
      puts  " => #{value.inspect}"
      #puts value
      @easiroc2SlowControl.store(key, value)
    end
  end
  ...
  def toTriggerDelay
    ...
    #delay3 = @triggerValues['DelayL1Trig'].to_i
	 # 20/03/03 require delay1 only if one wanna use it. 
	 #          the others are fixed internally. 
  ...
  ##### added 20/xx/xx
  def toInExTrigger
    intInExTrig = @triggerValues['InExTrigger']
  end
  ...
  def toUsrClkOutRegister
	...
	def toTestChargePtn
   	...
   	coeff = @registerValue['TestChargePtn']
	end
