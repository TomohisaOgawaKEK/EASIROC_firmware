...
class RBCP
   ...
   # WRITE= 0x80 #          128(10)  1000_0000(2)
	...
	def validate(rw, address, dataLength, data, receivedData)
      ...
      raise RBCPError.new("RBCPError Invalid ID")         if header.id != @id
      raise RBCPError.new("RBCPError Invalid DataLength") if header.dataLength != dataLength
      raise RBCPError.new("RBCPError Invalid Address")    if header.address != address
      raise RBCPError.new("RBCPError Frame Error")        if header.dataLength != receivedData.length - 8
    end
    private :validate
end
